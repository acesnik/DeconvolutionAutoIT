   ...
   If $NumberSpectraToAvg <> "" Then
	  ...
	  If $FinalSpect <> "" Then
		 ...
		 If $StartSpect <> "" Then
			...
			For $i = $StartSpect to $FinalSpect - $ScanInterval Step $ScanInterval ;$i is the final spectrum in the current range
			   ...
			   Else
				  $sep = ","
			   EndIf
			   Send("_Avg" & $i & $sep & ($i + $ScanInterval) & "{ENTER}", 0) ;enter spectrum numbers
